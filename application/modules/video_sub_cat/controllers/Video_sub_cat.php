<?php
class Video_sub_cat extends MX_Controller 
{
...
function delete($update_id)
{
    ...
    redirect('video_sub_cat/update/'.$item_id);
}
...
function submit($item_id)
{
    ...
    if ($submit=="Finished") {
        ...
        if ($cat_id!="") {
            ...
            $cat_title = $this->videocat->_get_cat_title($cat_id);
...
function update($item_id)
{
    ...
    $this->load->module('videocat');
    $sub_categories = $this->videocat->_get_all_sub_cats_for_dropdown();
    ...
    foreach($query->result() as $row) {
        $cat_title = $this->videocat->_get_cat_title($row->cat_id);
        $parent_cat_title = $this->videocat->_get_parent_cat_title($row->cat_id);
...
{
    $this->load->model('Mdl_video_sub_cat');
    $query = $this->Mdl_video_sub_cat->get($order_by);
    return $query;
}
...
function get_with_limit($limit, $offset, $order_by) 
{
    ...
    $this->load->model('Mdl_video_sub_cat');
    $query = $this->Mdl_video_sub_cat->get_with_limit($limit, $offset, $order_by);
    return $query;
}
...
function get_where($id)
{
    ...
    $this->load->model('Mdl_video_sub_cat');
    $query = $this->Mdl_video_sub_cat->get_where($id);
    return $query;
}
...
{
    $this->load->model('Mdl_video_sub_cat');
    $query = $this->Mdl_video_sub_cat->get_where_custom($col, $value);
    return $query;
}
...
{
    $this->load->model('Mdl_video_sub_cat');
    $this->Mdl_video_sub_cat->_insert($data);
}
...
function _update($id, $data)
{
    ...
    $this->load->model('Mdl_video_sub_cat');
    $this->Mdl_video_sub_cat->_update($id, $data);
}
...
function _delete($id)
{
    ...
    $this->load->model('Mdl_video_sub_cat');
    $this->Mdl_video_sub_cat->_delete($id);
}

function count_where($column, $value) 
{
    $this->load->model('Mdl_video_sub_cat');
    $count = $this->Mdl_video_sub_cat->count_where($column, $value);
    return $count;
}
...
{
    $this->load->model('Mdl_video_sub_cat');
    $max_id = $this->Mdl_video_sub_cat->get_max();
    return $max_id;
}
...
{
    $this->load->model('Mdl_video_sub_cat');
    $query = $this->Mdl_video_sub_cat->_custom_query($mysql_query);
    return $query;
}
