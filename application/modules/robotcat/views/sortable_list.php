...
<ul id="sortlist">
    ...
    foreach($query->result() as $row) { 
        $num_sub_cats = $this->robotcat->_count_sub_cats($row->id);
        $edit_item_url = base_url()."robotcat/create/".$row->id;
        $view_item_url = base_url()."robotcat/view/".$row->id;

        if ($row->robot_parent_cat_id==0) {
          $parent_cat_title = "&nbsp;";
        } else {
          $parent_cat_title = $this->robotcat->_get_cat_title($row->robot_parent_cat_id);
        }                            
    ?>
    ...
    <?= $parent_cat_title ?>
        ...
        if ($num_sub_cats<1) {
          ...
          $sub_cat_url = base_url()."robotcat/manage/".$row->id;
